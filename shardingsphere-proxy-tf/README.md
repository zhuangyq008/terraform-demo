...
* Terraform CLI
* AWS Resource
  * 安装AWS CLI 配置为AK/SK分配好IAM权限
  * 一个内部ss-proxy的域名 默认`shardingsphere.org`
  * 提供VPC，并至少垮两个AZ的子网，子网必须要能outbound访问到internet下载安装包
  * 安全组inbound的端口 2888, 3888, , 2181
  * 提供ssh keypair
...
| `<a name="input_hosted_zone_name"></a>` [hosted\_zone\_name](#input\_hosted\_zone\_name) | The name of the hosted private zone.The AWS private 'hosted_zone' must be created first, and the private zone is associated with the VPC, so the variable 'vpcid' must be matched to find the matching 'route53_hosted_zone' | `string`       | `"shardingsphere.org"`                                                              |    no    |
